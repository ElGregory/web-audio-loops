...
const Index = () => {
  ...
  const handleSequencerStep = (step: number, isActive: boolean) => {
    if (isActive && isInitialized) {
      // Vary frequency based on step for melodic patterns
      const stepFreq = synthParams.frequency * Math.pow(2, (step - 4) / 12); // Chromatic scale
      const stepParams = { ...synthParams, frequency: stepFreq };
